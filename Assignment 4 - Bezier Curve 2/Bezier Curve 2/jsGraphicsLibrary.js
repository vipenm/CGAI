...
function drawLine(x1, y1, x2, y2, clr, width) {
    "use strict";
...
/*
drawing State = 1 (Start of shape i.e. move to)
drawing State = 2 (just drawing of shape i.e. line to)
drawing State = 3 (end of shape i.e. move to)
image state = true (draw solid)
imageState = false (line drawing)
fillColour (RGB colour)
*/
function drawShape(x, y, drawingState, imageState, fillColour) {
    "use strict";
    ctx.strokeStyle = "black";
    if (drawingState === 1) {
        ctx.beginPath();
        ctx.moveTo(scaleX(Number(x)), scaleY(Number(y)));
    }
    if (drawingState === 2) {
        ctx.lineTo(scaleX(Number(x)), scaleY(Number(y)));
        ctx.stroke();
    }
    if (drawingState === 3) {
        ctx.lineTo(scaleX(Number(x)), scaleY(Number(y)));
...
//allows background colour of main grid area to change.
function changeBackgroundColour(clr) {
    "use strict";
    ctx.fillStyle = clr;
...
/*Generic function to display text
 x and y of text
 the actual text passed as a string
 colour of the text
 width of the text
 */
function displayText(xCoord, yCoord, text, clr, width) {
    "use strict";
    ctx.beginPath();
    ctx.strokeStyle = clr;
    ctx.lineWidth = width;
    ...
    //Label Graphs
    ctx.font = "9px Georgia";
    ctx.fillText(text, scaleX(Number(xCoord)), scaleY(Number(yCoord)));
}
